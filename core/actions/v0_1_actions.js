...
// REFRESH_RESUME_V1_IF_EXISTS_V1
// 目的：某些步骤（如 kash）保存后需要“刷新 V1 预览”（如果已有 V1）。
// 注意：IfExists => 没有 rv 就直接返回 null，不创建新 rv（避免流程跳步/污染状态）。
export function refreshResumeV1IfExists() {
  try {
    const db = loadDB();
    const sid = db?.active?.session_id;
    const rv =
      db?.active?.resume_version_id ||
      (sid ? db?.sessions?.[sid]?.resume_version_id : null);

    if (!rv) return null; // nothing to refresh
    return writeResumeV1Dummy(); // 复用既有 rv 的写入逻辑
  } catch (e) {
    console.warn("[actions] refreshResumeV1IfExists failed", e);
    return null;
  }
}
