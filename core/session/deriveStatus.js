  ...
  try {
    ...
    if (!sid || !hasId(db, "sessions", sid)) {
      return {
        ...
        error: makeError(ErrorCode.SESSION_NOT_FOUND, "Session not found", StepId.entry).error,
      };
    }
...
EXPORT_GENERATED: db?.export_ready === true,
  ...
  required_step: state,
  error: state === StepId.export ? null : makeError(ErrorCode.STEP_REQUIRED, "Step required", state).error,
};
  } catch (e) {
    return {
      state: StepId.entry,
