...
export function mountFactStructuredForm_v0_2_3({ hostId="fact_structured_host" } = {}){
  ...
  const state = {
    ...
    // [v0.2.3-14] personal summary (plain text)
    summary_text: "",
  ...
  async function hydrateFromDB(){
    try{
      ...
      state.summary_text = payload.summary_text || payload.summary || state.summary_text || "";
  ...
  if (state.education_items.length===0) state.education_items.push({ school:"", major:"", degree:"" });
      ...
      dom.summary.value = payload.summary_text || payload.summary || "";
  ...
  function sync(){
    ...
    // [v0.2.3-14] summary
    state.summary_text = s(dom.summary.value);
const payload = buildPayloadFromState(state);

      // ENSURE_BASIC_MIRROR_V1: force mirror basic/summary for downstream adapters
      payload.basic = { ...(payload.basic || {}), ...(state.basic || {}) };
      if (!payload.summary && payload.summary_text) payload.summary = payload.summary_text;
...
// 触发预览刷新（预览监听 #app 的 input/change，这里手动派发）
  ...
  const dom = {
    ...
    summary: uiTextarea("个人简介（可先简单写1-2句；自动生成后续做）", 3),
  };
  [dom.name,dom.target,dom.city,dom.phone,dom.email,dom.skills,dom.summary].forEach(x=>x.addEventListener("input", sync));
  ...
  basicBox.appendChild(inputRow("个人简介", dom.summary));
  host.appendChild(basicBox);

  // [v0.2.3-14] education section
  const eduHead = sectionHeader("教育背景", "一条=学校｜专业｜学历");
  const addEdu = button("+ 添加教育背景", ()=>{
    state.education_items.push({ school:"", major:"", degree:"" });
    render(); sync();
  });
  eduHead.appendChild(addEdu);
  host.appendChild(eduHead);
  const eduList = el("div");
  host.appendChild(eduList);

  function renderEducation(){
    eduList.innerHTML = "";
    state.education_items.forEach((it, idx)=>{
      const { box, head } = cardShell(`教育背景 #${idx+1}`);
      const del = button("删除", ()=>{
        if (state.education_items.length<=1) return;
        state.education_items.splice(idx,1); render(); sync();
      });
      head.appendChild(del);

      const school = uiInput("学校");
      school.value = it.school || "";
      school.addEventListener("input", ()=>{ it.school=s(school.value); sync(); });

      const major = uiInput("专业");
      major.value = it.major || "";
      major.addEventListener("input", ()=>{ it.major=s(major.value); sync(); });

      const degree = uiInput("学历");
      degree.value = it.degree || "";
      degree.addEventListener("input", ()=>{ it.degree=s(degree.value); sync(); });

      box.appendChild(inputRow("学校", school));
      box.appendChild(inputRow("专业", major));
      box.appendChild(inputRow("学历", degree));
...
renderExperience();
  ...
  // [v0.2.3-14] education + summary enabled
  })();
}
