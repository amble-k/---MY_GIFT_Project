/**
 * FACT Preview (v0.2.3) — row-based input (recommended)
...
function splitSkills(txt){
  const t = s(txt);
  if (!t) return [];
  return t.split(/[,，\n]/g).map(x=>x.trim()).filter(Boolean);
}

// ---- v0.2.3-07 row parsers ----
// 支持：| 或 tab 分列
function splitCols(line){
  const raw = String(line||"").trim();
  if (!raw) return [];
  const cols = raw.split(/\s*\|\s*|\t+/g).map(x=>x.trim()).filter(Boolean);
  return cols;
}

// projects: "主导 增长实验平台" / "参与 增长实验平台" / "增长实验平台"
...
// experience col2: "某公司/产品经理" 或 "某公司-产品经理" 或 "产品经理"
function parseCompanyAndTitle(col2){
  ...
  if (t.includes("-")){
    const [a,b] = t.split("-", 2);
    // 简单启发：两边都有长度才认为是公司-职位
    if (s(a).length >= 2 && s(b).length >= 2) return { company:s(a), title:s(b) };
  }
  return { company:"", title:t };
}

function parseExperienceRows(txt){
  // 每行：时间 | 职位(可写公司/职位) | 职责 | 成果
  const lines = splitLines(txt);
  const out = [];
  for (const ln of lines){
    const c = splitCols(ln);
    const period_text = c[0] || "";
    const { company, title } = parseCompanyAndTitle(c[1] || "");
    const resp = c[2] ? [c[2]] : [];
    const achv = c[3] ? [c[3]] : [];
...
function parseProjectRows(txt){
  // 每行：时间 | 项目职位(主导/参与+项目名) | 职责 | 成果
  const lines = splitLines(txt);
  const out = [];
  for (const ln of lines){
    const c = splitCols(ln);
    const period_text = c[0] || "";
    const { role, name } = parseRoleAndName(c[1] || "");
    const work = c[2] ? [c[2]] : [];
    const achv = c[3] ? [c[3]] : [];
    out.push({
      name,
...
function parseTrainingRows(txt){
  ...
  for (const ln of lines){
    ...
    const org = c[1] || "";
    const content = c[2] || "";
    if (!date_or_period && !org && !content) continue;
    out.push({ name: "培训", org, date_or_period, content, outcome: null });
  }
  return out;
}

// ---- input payload builder (v0.2.3) ----
function buildInputPayload_v0_2_3(){
  ...
  const certifications = certRowsTxt ? parseCertRows(certRowsTxt) : (() => {
    ...
    if (!name) return [];
    return [{ name, issuer: v("cert_issuer"), date: v("cert_date"), credential_url: null }];
  })();

  const trainings = trRowsTxt ? parseTrainingRows(trRowsTxt) : (() => {
    const name = v("tr_name");
    if (!name) return [];
    return [{ name, org: v("tr_org"), date_or_period: v("tr_period"), content: v("tr_content"), outcome: null }];
  })();
...
export function enableFactPreview_v0_2_3({ appId="app", locale="zh-CN", theme="professional_clean" } = {}){
  ...
  function render(){
    const input = (window.__INPUT_PAYLOAD_V0_2_3__ && typeof window.__INPUT_PAYLOAD_V0_2_3__ === 'object') ? window.__INPUT_PAYLOAD_V0_2_3__ : buildInputPayload_v0_2_3();
    ...
    host.innerHTML = `
      <div style="margin:0 auto;max-width:920px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin:0 0 10px 0;padding:10px 12px;border:1px solid #e9e9e9;border-radius:14px;background:#fff;">
          ...
          <div style="font-size:12px;color:rgba(0,0,0,0.55);">推荐：行录入（时间｜职位/项目职位｜职责｜成果）</div>
        </div>
        ${body}
      </div>
    `;
