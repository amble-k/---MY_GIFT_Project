...
import { RouteMap } from "../../core/session/step_ids.js?v=20260106_172219";
...
export function dispatch(ctx = {}) {
  ...
  // ENTRY_HUB_V3: /resume 作为入口 Hub（默认停在入口页，不自动推进）
// - 若需要自动推进到 required_step（仅调试/测试），请用 ?autogo=1
let autoGo = false;
try { autoGo = new URLSearchParams(location.search).get("autogo") === "1"; } catch { autoGo = false; }

if (currentRoute === "/resume" && autoGo) {
  const r2 = (st?.required_step && RouteMap?.[st.required_step])
...
if (shouldGuard) {
  ...
  console.log('[router][dispatch]', { currentRoute, targetRoute, pageFile, desiredUrl, pathname: location.pathname, hash: location.hash, shouldGuard, isHub, noGuard, required_step: st?.required_step, state: st?.state, error: st?.error?.code });

    const needMoveFile = location.pathname !== pageFile;
  const needFixHash = location.hash !== "#" + targetRoute;
  // ---- DEBUG JSON (for copy/paste) ----
  try {
    const dbg = {
      href: location.href,
      ...
      noGuard,
      shouldGuard,
      st_state: st?.state || null,
      st_required_step: st?.required_step || null,
      st_error_code: st?.error?.code || null,
    };
    window.__ROUTER_LAST__ = dbg;
    console.log("[router][dispatch]", dbg);
    console.log("[router][dispatch.json]", JSON.stringify(dbg, null, 2));
  } catch (e) {
    console.warn("[router][dispatch.json] failed", e);
  }


  
    const dbg = (()=>{ try { return new URLSearchParams(location.search).get("debug")==="1"; } catch { return false; } })();
    if (dbg) {
